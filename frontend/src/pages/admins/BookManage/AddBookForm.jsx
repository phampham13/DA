  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      categoryName: Yup.string().required(
        "Bạn chưa thêm danh mục cho sản phẩm"
      ),
      coverImg: Yup.string().required("Bạn chưa thêm ảnh bìa cho sách"),
      quantityTotal: Yup.number()
        .integer("Số lượng phải là số nguyên")
        .min(1, "Tổng sách không nhỏ hơn 1")
        .required("Nhập tổng số lượng sách"),
      quantityAvailabel: Yup.number()
        .integer("Số lượng phải là số nguyên")
        .min(0, "Số lượng không âm")
        .required("Nhập số lượng sách sẵn có")
        .test(
          "is-less-or-equal",
          "Số lượng sẵn có phải nhỏ hơn hoặc bằng tổng số lượng",
          function (value) {
          ...
          <div className={cx("form-group")}>
            ...
            <div className={cx("form-input")} id={cx("quantityTotal")}>
              ...
              <input
                ...
                value={formik.values.quantityTotal}
                onChange={formik.handleChange}
                className={cx("form-control")}
              />
              {formik.errors.quantityTotal && formik.touched.quantityTotal && (
                <span className={cx("form-message")}>
                  {formik.errors.quantityTotal}
                </span>
              )}
            </div>
            <div className={cx("form-input")} id={cx("quantityAvailabel")}>
              ...
              <input
                id="quantityAvailabel"
                name="quantityAvailabel"
                type="number"
                placeholder="nhập số lượng"
                value={formik.values.quantityAvailabel}
                onChange={formik.handleChange}
                className={cx("form-control")}
              />
              {formik.errors.quantityAvailabel &&
                formik.touched.quantityAvailabel && (
                  <span className={cx("form-message")}>
                    {formik.errors.quantityAvailabel}
                  </span>
                )}
